...
- **アーティファクト抽出 & 範囲選択** *(v0.5.0+)*：claude.ai キャプチャで「**アーティファクトだけ**」（Claude が書いたコード／ドキュメントを整形済みコードブロックで抽出、会話は捨てる）や「**直近 N 件のみ**」を選択可能。長い設計会話から必要な部分だけを取り込めます。アーティファクトは同一 ID の更新を最新版に集約
